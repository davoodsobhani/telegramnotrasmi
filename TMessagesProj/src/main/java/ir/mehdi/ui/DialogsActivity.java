...
import ir.mehdi.messenger.ChatObject;
import ir.mehdi.messenger.ContactsController;
import ir.mehdi.messenger.DialogObject;
import ir.mehdi.messenger.FileLog;
import ir.mehdi.messenger.ImageLoader;
import ir.mehdi.messenger.LocaleController;
import ir.mehdi.messenger.MessageObject;
import ir.mehdi.messenger.MessagesController;
import ir.mehdi.messenger.MessagesStorage;
import ir.mehdi.messenger.NotificationCenter;
import ir.mehdi.messenger.R;
import ir.mehdi.messenger.UserConfig;
import ir.mehdi.messenger.UserObject;
import ir.mehdi.messenger.query.SearchQuery;
import ir.mehdi.messenger.query.StickersQuery;
import ir.mehdi.messenger.support.widget.LinearLayoutManager;
import ir.mehdi.messenger.support.widget.RecyclerView;
import ir.mehdi.telegraf.FavoriteController;
import ir.mehdi.telegraf.HiddenController;
import ir.mehdi.telegraf.OnSwipeTouchListener;
import ir.mehdi.telegraf.Setting;
import ir.mehdi.telegraf.TabSetting;
import ir.mehdi.telegraf.ThemeChanger;
import ir.mehdi.tgnet.TLRPC;
import ir.mehdi.ui.ActionBar.ActionBar;
import ir.mehdi.ui.ActionBar.ActionBarMenu;
import ir.mehdi.ui.ActionBar.ActionBarMenuItem;
import ir.mehdi.ui.ActionBar.AlertDialog;
import ir.mehdi.ui.ActionBar.BaseFragment;
import ir.mehdi.ui.ActionBar.BottomSheet;
import ir.mehdi.ui.ActionBar.MenuDrawable;
import ir.mehdi.ui.ActionBar.Theme;
import ir.mehdi.ui.ActionBar.ThemeDescription;
import ir.mehdi.ui.Adapters.DialogsAdapter;
import ir.mehdi.ui.Adapters.DialogsSearchAdapter;
import ir.mehdi.ui.Cells.DialogCell;
import ir.mehdi.ui.Cells.DividerCell;
import ir.mehdi.ui.Cells.DrawerActionCell;
import ir.mehdi.ui.Cells.DrawerProfileCell;
import ir.mehdi.ui.Cells.GraySectionCell;
import ir.mehdi.ui.Cells.HashtagSearchCell;
import ir.mehdi.ui.Cells.HintDialogCell;
import ir.mehdi.ui.Cells.LoadingCell;
import ir.mehdi.ui.Cells.ProfileSearchCell;
import ir.mehdi.ui.Cells.UserCell;
import ir.mehdi.ui.Components.CombinedDrawable;
import ir.mehdi.ui.Components.EmptyTextProgressView;
import ir.mehdi.ui.Components.FragmentContextView;
import ir.mehdi.ui.Components.LayoutHelper;
import ir.mehdi.ui.Components.RadialProgressView;
import ir.mehdi.ui.Components.RecyclerListView;
		...
		implements NotificationCenter.NotificationCenterDelegate
{
	...
	private ActionBarMenuItem zangoolehMenu; // telegraf
	private BadgeView         zangoolehBadge; // telegraf
	private Handler           zangoolehHandler; // telegraf
	private Runnable          zangoolehRunnable; // telegraf
	...
	public View createView(final Context context)
	{
		...
		// telegraf ------------- Search Start ----------------
		searchFiledItem = menu.addItem(0, R.drawable.ic_ab_search).setIsSearchField(true).setActionBarMenuItemSearchListener(new ActionBarMenuItem.ActionBarMenuItemSearchListener() // telegraf
		{
			@Override
			...
			public void onSearchCollapse()
			{
				...
				if (listView != null)
				{
					...
					if (MessagesController.getInstance().loadingDialogs && MessagesController.getInstance().dialogs.isEmpty()) // telegraf
					{
						emptyView.setVisibility(View.GONE);
			...
			public void onTextChanged(EditText editText)
			{
				...
				if (!enterHiddenPassMode) // telegraf
				{
				...
				else if (Setting.CheckHidePassword(text)) // telegraf
				{
		...
		searchFiledItem.getSearchField().setHint(LocaleController.getString("Search", R.string.Search)); // telegraf
		// telegraf ------------- Search End ----------------

		// telegraf ------------- Ghost Start ----------------
		/*
		...
		*/
		...
		hiddenMode = ApplicationLoader.GetHiddenMode(context); // telegraf
		...
		listView.setTag(4);
		listView.setHasFixedSize(true); // telegraf
		listView.setBackgroundColor(0xffffffff); // telegraf
		...
		if (isTabsUpside)
		{
			frameLayout.addView(listView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT, Gravity.RIGHT, 0, 34, 0, 0)); // telegraf changed 48 to 34
		}
		else
		{
...
//				TabSetting.SetTabIcon(tab, TabSetting.getNormalIcon(tab.getPosition()));
		...
		if (isTabsUpside)
		{
			frameLayout.addView(tabHost, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 34, Gravity.TOP)); // telegraf Changed Wrap_Content to 34
		}
		else
		{
		...
		onItemListenerForDialogs = new RecyclerListView.OnItemClickListener()
		{
			...
			public void onItemClick(View view, int position)
			{
				...
				if (adapter == dialogsAdapter)
				{
					...
					dialog_id = dialog.id;
					MessagesController.getInstance().dialogsUnreadOnly.remove(dialog); // telegraf
				}
		...
		onItemListenerLongForDialogs = new RecyclerListView.OnItemLongClickListener()
		{
			...
			public boolean onItemClick(View view, int position)
			{
				...
				if (DialogObject.isChannel(dialog))
				{
					...
					CharSequence     items[];
					final boolean    isFavor  = FavoriteController.isFavor(selectedDialog); // telegraf
					final boolean    isHidden = HiddenController.IsHidden(selectedDialog);  // telegraf
					int icons[] = new int[]{
							R.drawable.ic_star_gray_24dp, // telegraf
							R.drawable.ic_settings_white_24dp, // telegraf
					...
					if (chat != null && chat.megagroup)
					{
						items = new CharSequence[]{
								isFavor ? LocaleController.getString("RemoveFromFavorites", R.string.RemoveFromFavorites) : LocaleController.getString("AddToFavorites", R.string.AddToFavorites), // telegraf
								isHidden ? LocaleController.getString("RemoveFromHiddens", R.string.RemoveFromHiddens) : LocaleController.getString("AddToHiddens", R.string.AddToHiddens), // telegraf
					...
					builder.setItems(items, icons, new DialogInterface.OnClickListener()
					{
						...
						public void onClick(DialogInterface dialog, final int which)
						{
							...
							listView.getAdapter().notifyDataSetChanged(); // telegraf
						}
					});
					showDialog(builder.create());
				}
		...
		emptyView.setOnTouchListener(onSwipeTouchListener); // telegraf
		...
		emptyTextView1.setTypeface(AndroidUtilities.getTypeface("fonts/rmedium.ttf")); // telegraf
		...
		emptyTextView2.setTypeface(AndroidUtilities.getTypeface("fonts/rmedium.ttf")); // telegraf
		...
		ThemeChanger.setFloatingbutton(floatingButton); // telegraf
		floatingButton.setVisibility(onlySelect ? View.GONE : View.VISIBLE);
		floatingButton.setScaleType(ImageView.ScaleType.CENTER);
		floatingButton.setBackgroundDrawable(ThemeChanger.getFloating(Setting.getActionbarcolor())); // telegraf
		floatingButton.setImageResource(R.drawable.floating_pencil); // telegraf
		...
		//		floatingButton.setBackgroundDrawable(drawable); // telegraf Commented
		//		floatingButton.setColorFilter(new PorterDuffColorFilter(Theme.getColor(Theme.key_chats_actionIcon), PorterDuff.Mode.MULTIPLY)); // telegraf Commented
		//		floatingButton.setImageResource(R.drawable.floating_pencil); // telegraf Commented
		if (Build.VERSION.SDK_INT >= 21)
		{
		...
		frameLayout.addView(floatingButtonLock, LayoutHelper.createFrame(Build.VERSION.SDK_INT >= 21 ? 56 : 60, Build.VERSION.SDK_INT >= 21 ? 56 : 60, (LocaleController.isRTL ? Gravity.LEFT : Gravity.RIGHT) | Gravity.BOTTOM, LocaleController.isRTL ? 14 : 0, 0, LocaleController.isRTL ? 0 : 14, (isTabsUpside ? 74 : 110))); // telegraf
		...
		if (searchString == null)
		{
			...
			dialogsAdapter.categoryId = TabSetting.getTabModels().get(0).getId(); // telegraf
			dialogsAdapter.Hiddenmode = hiddenMode; // telegraf
			listView.setAdapter(dialogsAdapter);
		}
		int type = 0;
		if (searchString != null)
...
//		TabSetting.startThread(); // telegraf
	}
	...
	private ArrayList<TLRPC.TL_dialog> getDialogsArray()
	{
		...
		{
			dialogsAdapter.Hiddenmode = hiddenMode; // telegraf
			return dialogsAdapter.getDialogsArray(); // telegraf
		}
