...
public class DialogCell extends BaseCell
{
	...
	private int           index;
	private int           dialogsType; // telegraf
	private boolean drawStatus = true; // telegraf
	private GradientDrawable statusBG; // telegraf
	private String           lastStatus; // telegraf
	private boolean          hiddenmode; // telegraf
	...
	private boolean setStatusColor()
	{
		...
		{
			this.statusBG.setVisible(false, true);
		}
		...
		{
			this.statusBG.setColor(Color.parseColor("#fb9800"));
		}
		else
		{
			this.statusBG.setColor(Color.GRAY);
		}
		return true;
	}
	...
	public void setDialog(TLRPC.TL_dialog dialog, int i, int type, boolean hiddenmode)
	{
		currentDialogId = dialog.id;
		isDialogCell = true;
		index = i;
		dialogsType = type;
		this.hiddenmode = hiddenmode; // telegraf
		update(0);
	}
	...
	protected void onDraw(Canvas canvas)
	{
		...
		// telegraf Start
		if (this.drawStatus && !drawNameBroadcast && !drawNameGroup)
		{
			int measuredWidth;
			if (LocaleController.isRTL)
			{
			...
			setDrawableBounds(this.statusBG, measuredWidth, AndroidUtilities.dp(46.0f), AndroidUtilities.dp(12.0f), AndroidUtilities.dp(12.0f));
