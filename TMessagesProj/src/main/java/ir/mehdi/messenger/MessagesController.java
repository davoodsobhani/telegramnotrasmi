...
import ir.mehdi.SQLite.SQLiteCursor;
...
import ir.mehdi.tgnet.NativeByteBuffer;
import ir.mehdi.tgnet.RequestDelegate;
import ir.mehdi.tgnet.SerializedData;
...
import ir.mehdi.ui.ChatActivity;
import ir.mehdi.ui.Components.AlertsCreator;
import ir.mehdi.ui.ProfileActivity;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate
{
	...
	public                  ArrayList<TLRPC.TL_dialog>  dialogsChannelOnly              = new ArrayList<>(); //telegraf
	public                  ArrayList<TLRPC.TL_dialog>  dialogsJustGroupsOnly           = new ArrayList<>(); //telegraf
	public                  ArrayList<TLRPC.TL_dialog>  dialogsContactOnly              = new ArrayList<>(); //telegraf
	...
	public void cleanup()
	{
		...
		fullUsers.clear();
		dialogs.clear();
		dialogsChannelOnly.clear(); //telegraf
		dialogsJustGroupsOnly.clear(); //telegraf
		dialogsContactOnly.clear(); //telegraf
		dialogsFavoriteOnly.clear(); //telegraf
		dialogsBotOnly.clear(); //telegraf
		...
		if (first)
		{
			...
			if (dialog != null)
			{
				...
				if (onlyHistory == 0 || onlyHistory == 3)
				{
					...
					dialogsChannelOnly.remove(dialog); //telegraf
					dialogsJustGroupsOnly.remove(dialog); //telegraf
					dialogsContactOnly.remove(dialog); //telegraf
					dialogsFavoriteOnly.remove(dialog); //telegraf
					dialogsBotOnly.remove(dialog); //telegraf
					dialogsUnreadOnly.remove(dialog); //telegraf
	...
	private void deleteADialog(TLRPC.TL_dialog dialog) //telegraf
	{
		dialogsChannelOnly.remove(dialog); //telegraf
		dialogsJustGroupsOnly.remove(dialog); //telegraf
		dialogsContactOnly.remove(dialog); //telegraf
			...
			if (ConnectionsManager.getInstance().getPauseTime() == 0 && ApplicationLoader.isScreenOn && !ApplicationLoader.mainInterfacePausedStageQueue)
			{
				if (ApplicationLoader.mainInterfacePausedStageQueueTime != 0 && Math.abs(ApplicationLoader.mainInterfacePausedStageQueueTime - System.currentTimeMillis()) > 1000)
				{
					if (statusSettingState != 1 && (lastStatusUpdateTime == 0 || Math.abs(System.currentTimeMillis() - lastStatusUpdateTime) >= 55000 || offlineSent))
					{
						...
						req.offline = Setting.getGhostMode(); //telegraf
			...
			public void run()
			{
				...
				AndroidUtilities.runOnUIThread(new Runnable()
				{
					...
					public void run()
					{
						...
						dialogsFavoriteOnly.clear(); //telegraf
						dialogsBotOnly.clear(); //telegraf
						dialogsUnreadOnly.clear(); //telegraf
						dialogs.clear();
	...
	{
		Log.d("_telegraf", "MsgContoller sortDialogs");
		...
		for (int a = 0; a < dialogs.size(); a++)
		{
			...
			if (lower_id != 0 && high_id != 1)
			{
				...
				if (DialogObject.isChannel(d))
				{
					...
					if (chat != null && chat.megagroup) // telegraf, this is super group
					{
						dialogsGroupsOnly.add(d); // telegraf
					}
					if (chat != null && !chat.megagroup) // telegraf
					{
						dialogsChannelOnly.add(d); // telegraf
					}
				}
				else if (lower_id < 0)
				{
					...
					dialogsGroupsOnly.add(d);
					dialogsJustGroupsOnly.add(d); // telegraf
				}
				else if (lower_id > 0) // telegraf
				{
